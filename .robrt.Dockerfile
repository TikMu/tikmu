FROM ubuntu:15.10
ENV PS1="# "
RUN apt-get update && apt-get install -y git build-essential && mkdir -p /var/git
...
RUN git clone --recursive --branch master --depth 1 https://github.com/waneck/erazor.git /var/git/erazor && haxelib dev erazor /var/git/erazor
