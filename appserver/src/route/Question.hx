...
class SomeQuestion extends BaseRoute {
	...
	public function postAnswer(args:{ answer:String })
	{
		...
		_ctx.scheduleDeferred(_ctx.reputation.update.bind({ value : RPostAnswer, target : RAnswer(ans, question) }));
	...
	public function postFavorite()
	{
		...
		if (!loop.session.user.equals(question.user)) {
			for (e in events)
				_ctx.scheduleDeferred(_ctx.reputation.update.bind({ value : e, target : RQuestion(question) }));
		}
