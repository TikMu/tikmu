...
class ListRoute extends BaseRoute
{
	...
	public function anyDefault():HttpResponse<{ qs:Array<{id : ObjectID, userID : ObjectID, userName : String, contents : String, tags : Array<String>, loc : db.helper.Location, voteSum : Int, favorites : Int, watchers : Int, date : Date, solved : Bool, answersCount : Int}>, authenticated : Bool, myUser : Null<ObjectID> }>
	{
		...
		var qs = [ for (q in ctx.questions.find({})) { trace(q);trace(q.user);trace(q.user.get); { id : q._id, userID : q.user.get(ctx.users.col)._id, userName : q.user.get(ctx.users.col).name, contents : q.contents, tags : q.tags, loc : q.loc, voteSum : q.voteSum, favorites : q.favorites, watchers : q.watchers, date : q.created, solved : q.solved, answersCount : q.answers.length }; } ];
...
class ListView extends erazor.macro.SimpleTemplate<{ qs:Array<{id : ObjectID, userID : ObjectID, userName : String, contents : String, tags : Array<String>, loc : db.helper.Location, voteSum : Int, favorites : Int, watchers : Int, date : Date, solved : Bool, answersCount : Int}>, authenticated : Bool, myUser : Null<ObjectID> }>
{
}
